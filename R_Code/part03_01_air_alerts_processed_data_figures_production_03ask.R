...
# Fix 2025.03.18.
# Reading new and old regions match file
load(file = "../R_Data/part01_06_air_raid_sirens_official_data_kharkivska_oblast_aggregated.RData")
...
# plot configurations
# Path
master_file_all_infections_plots_configuration_path <- paste0("../Data/master_file_all_infections_plots_configuration_alerts.xlsx")
# Actual file
...
for( infection_index in c(1:dim(diseases_names_master_file)[1]) )
{
  ...
  # Fix 2025.03.17.
  # Extracting dates past Jan 1, 2022 only to align with air alerts.
  dataset_current <- dataset_current[ dataset_current$period_last_date >= as.Date("2022-02-01"), ]
  ...
  pdf(paste("../Plots/part03_01_figure_raw_data_", infection_current, ".pdf", sep = ""), height = 12, width = 28)
  ...
  for(region_current_name in regions_unique_rearranged_short)
  {
    ...
    # Fix 2025.03.18.
    # Getting data alerts for the given region
    air_alerts_current_region <- 
      part01_06_air_raid_sirens_official_data_kharkivska_oblast_aggregated[part01_06_air_raid_sirens_official_data_kharkivska_oblast_aggregated$districts_created_since_2021_shp == region_current_name,]
    # Aligning the range of the alerts dataset.
    air_alerts_current_region <- air_alerts_current_region[air_alerts_current_region$period_last_date <= max(dataset_current_region$period_last_date),]
    air_alerts_current_region <- air_alerts_current_region[air_alerts_current_region$period_last_date >= war_start,]
    ...
    corr_actual  <- round( x = cor(x = dataset_current_region$total_cases, y = air_alerts_current_region$total_hours), digits = 2)
    corr_absolut <- round( x = abs(corr_actual), digits = 2)
    

    # loess 
    # dataset_current_region
    dataset_current_region_loess <- loess( total_cases ~ as.integer(period_last_date), data = dataset_current_region, span = 0.50 )
    dataset_current_region$total_cases_loess <- predict(dataset_current_region_loess)
    ...
    # lines(
    #   x = dataset_current_region$period_last_date[dataset_current_region$period_last_date >= war_start],
    #   y = dataset_current_region$total_cases_loess[dataset_current_region$period_last_date >= war_start],
    #   # col = "#238443",
    #   col = "#72c097",
    #   # col = color_01,
    #   lwd = 5,
    ...
    # Fix 2025.03.18.
    # Plotting air alerts
    incidence_limits <-  c( min(dataset_current_region$total_cases), max(dataset_current_region$total_cases) * 1.01  )
    
    # Re-scaling total hours for a given region to 0-1 interval.
    air_alerts_current_region$total_hours_rescaled <-  air_alerts_current_region$total_hours/max(air_alerts_current_region$total_hours)
    
    # Putting them on the same scale 
    range_new <- incidence_limits[2] - incidence_limits[1]
    # Final values
    air_alerts_current_region$total_hours_final <-  air_alerts_current_region$total_hours_rescaled * range_new + incidence_limits[1]
    
    
    # loess 
    # air_alerts_current_region
    air_alerts_current_region_loess_final <- loess( total_hours_final ~ as.integer(period_last_date), data = air_alerts_current_region, span = 0.50 )
    air_alerts_current_region$total_hours_final_loess <- predict(air_alerts_current_region_loess_final)
    
    

    lines(
      x = air_alerts_current_region$period_last_date,
      y = air_alerts_current_region$total_hours_final,
      # col = "#238443",
      col = "#5C4033",
      # col = color_01,
      lwd = 5,
    ...
    # lines(
    #   x = air_alerts_current_region$period_last_date,
    #   y = air_alerts_current_region$total_hours_final_loess,
    #   # col = "#238443",
    #   col = "#d08956",
    #   # col = color_01,
    #   lwd = 5,
    ...
    #   inset = c(plot_characteristics_current_disease_current_region$coordinate1_a, plot_characteristics_current_disease_current_region$coordinate2_a),
    #   legend = c("Wartime Data", "Wartime Data (loess)", "Air Alerts", "Air Alerts (loess)"),
    #   col = "black",   
    #   fill = c( "#d9f0a3", "#5C4033", "#72c097", "#d08956"),
    #   pt.cex = c(4, 2),
    #   # pch = c(19, 20),
    #   # pch = c(19, 20),  
    #   # pch = c(19, 20),
    #   cex = plot_characteristics_current_disease_current_region$legend_cex_a
    # )
    ...
    legend(
      x = plot_characteristics_current_disease_current_region$location,
      inset = c(plot_characteristics_current_disease_current_region$coordinate1_a, plot_characteristics_current_disease_current_region$coordinate2_a),
      legend = c("Wartime Data", "Air Alerts" ),
      col = "black",   
      fill = c( "#d9f0a3", "#5C4033" ),
      pt.cex = c(4, 2),
      # pch = c(19, 20),
      # pch = c(19, 20),  
      # pch = c(19, 20),
    ...
    #   legend = c("Pre-War Data", "Wartime Data", "War Start", "Air Alerts"),
    #   col = "black",   
    #   fill = c("#238443", "#d9f0a3", "red", "#5C4033"),
    #   pt.cex = c(4, 2),
    #   # pch = c(19, 20),
    #   # pch = c(19, 20),  
    #   # pch = c(19, 20),
    ...
    legend(x = plot_characteristics_current_disease_current_region$location,
           inset = c(plot_characteristics_current_disease_current_region$coordinate1_b, plot_characteristics_current_disease_current_region$coordinate2_b ),
           legend = c( as.expression(bquote(r ~ " = " ~ .(corr_actual))),
                       as.expression(bquote(r ~ "(abs) = " ~ .(corr_absolut)))), 
           cex = plot_characteristics_current_disease_current_region$legend_cex_b,
           pt.cex = 0.0001,
           box.col=0, 
           bty="n")
    ...
    x_indexes_to_display <- 
      dataset_current_region$period_last_date[seq( from  =  1, to = length(dataset_current_region$period_last_date),  by = 2 )]
    ...
    x_lablist <- 
      dataset_current_region$period[seq( from  =  1, to = length(dataset_current_region$period_last_date),  by = 2 )]
    ...
    # Adding axis label
    # labels FAQ -> https://stackoverflow.com/questions/26180178/r-boxplot-how-to-move-the-x-axis-label-down
    y_min_value_labels <- min(air_alerts_current_region$total_hours)
    y_max_value_labels <- max(air_alerts_current_region$total_hours)
    y_tlab_labels    <- seq(from = y_min_value_labels, to = y_max_value_labels, by = (y_max_value_labels - y_min_value_labels) / 5)
    

    y_min_value_values <- min(air_alerts_current_region$total_hours_final)
    y_max_value_values <- max(air_alerts_current_region$total_hours_final)
    y_tlab_values    <- seq(from = y_min_value_values, to = y_max_value_values, by = (y_max_value_values - y_min_value_values) / 5)
    
    
    
    y_lablist <- as.character(round(y_tlab_labels, digits = 0))
    
    # Fix 2025.03.18.
    # For the case when all values are zero. Rewrite with zeroes.
    if (length(y_tlab_values) == 1)
    { 
      y_lablist <- y_tlab_values  
      
    # End of -> if (length(y_tlab_values) == 1)   
    } 

    axis(4, at = y_tlab_values, labels = y_lablist, cex.axis = 1.5)
