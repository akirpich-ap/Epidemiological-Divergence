...
# library(forecast) - library for time series forecasting using ARIMA.
library(forecast)
...
# library("arima") - library for time series forecasting.
# library("arima")
...
# Fix 2024.12.10.
# Fitting a arima model in the loop
...
# ARIMA Tweak
#diseases_names_master_file <- diseases_names_master_file[diseases_names_master_file$short_name_part1 %in% c("enter_colit_gastroent_fdpoison", "salmonellosis", "acute_resp_multi_unspecified_any"), ]
diseases_names_master_file <- 
  diseases_names_master_file[!diseases_names_master_file$short_name_part1 %in% c("salmonellosis_social_groups", "enterit_yersin_enterocol", "incl_campylobact_enteritis"), ]
...
for (box_cox_local in box_cox_local_list )
{
  ...
  for( infection_index in c(1:dim(diseases_names_master_file)[1]) )
  {
    ...
    pdf(paste("../Plots/part02_03_arima_", infection_current, "_inc_box_cox_", box_cox_local, ".pdf", sep = ""), height = 12, width = 28)
    ...
    for(region_current_name in regions_unique_rearranged_short)
    {
      ...
      # Creating an arima object.
      # arima_object_total_cases_object <- arima(data_to_feed_truncated_total_cases$y, order = c(6, 1, 6) )
      # arima_object_total_cases_object <- auto.arima(data_to_feed_truncated_total_cases$y )
      arima_object_total_cases_object <- arima(data_to_feed_truncated_total_cases$y, order = c(9, 1, 4), method = "ML" ,
      ...
      forecasted_values <- forecast( arima_object_total_cases_object, h =  dim(data_to_feed_full_total_cases)[1] - dim(data_to_feed_truncated_total_cases)[1] )
      arima_predictions_total_cases <- 
        data.frame( yhat = c( as.vector(fitted(arima_object_total_cases_object)), as.vector(forecasted_values$mean) )  )
      # Fixing dates
      arima_predictions_total_cases$ds <- as.Date(data_to_feed_full_total_cases_dates_only$ds)
      ...
      # print(cutoffs)
      # cv_results = cross_validation(arima_object_total_cases, period=30, initial = 365*4, horizon=horizon, units="days")
      # arima_predictions_total_cases_plus_original_data_diag = performance_metrics(cv_results)
      # save(arima_predictions_total_cases_plus_original_data_diag, file = paste("../../R_Data/arima_predictions_total_cases_plus_original_data_diag.RData"))
      
      
      # Adding original data
      arima_predictions_total_cases_plus_original_data <- base::merge(x = data_to_feed_full_total_cases, y = arima_predictions_total_cases, by = "ds")
      dim(arima_predictions_total_cases_plus_original_data)
      
      
      # Summaries for mortalities
      dim(arima_predictions_total_cases_plus_original_data)
      head(arima_predictions_total_cases_plus_original_data)
      tail(arima_predictions_total_cases_plus_original_data)
      ...
      if(box_cox_local == 1){ # Runs if box-cox option was selected
        ...
        # y
        arima_predictions_total_cases_plus_original_data$y_original <- InvBoxCox(x = arima_predictions_total_cases_plus_original_data$y, lambda = lam) - 1 
        
        # y-hat 
        arima_predictions_total_cases_plus_original_data$yhat_original <- InvBoxCox(x = arima_predictions_total_cases_plus_original_data$yhat, lambda = lam) - 1 
        
        # Lower Bound 
        # arima_predictions_total_cases_plus_original_data$yhat_lower_original <- InvBoxCox(x = arima_predictions_total_cases_plus_original_data$yhat_lower, lambda = lam) - 1
        
        # Upper Bound 
        # arima_predictions_total_cases_plus_original_data$yhat_upper_original <- InvBoxCox(x = arima_predictions_total_cases_plus_original_data$yhat_upper, lambda = lam) - 1
        
      }else{ # Prints if no box-cox option was selected
        
        print("No Box-Cox Transformation Selected. -> 10000 ^ (result) - 1 is used")
        
        # y
        arima_predictions_total_cases_plus_original_data$y_original <- 10000^(arima_predictions_total_cases_plus_original_data$y) - 1
        
        # y-hat 
        arima_predictions_total_cases_plus_original_data$yhat_original <- 10000^(arima_predictions_total_cases_plus_original_data$yhat) -1
        
        # Lower Bound 
        # arima_predictions_total_cases_plus_original_data$yhat_lower_original <- 10000^(arima_predictions_total_cases_plus_original_data$yhat_lower) - 1 
        
        # Upper Bound 
        # arima_predictions_total_cases_plus_original_data$yhat_upper_original <- 10000^(arima_predictions_total_cases_plus_original_data$yhat_upper) - 1
      ...
      plot(
        # x = arima_predictions_total_cases_plus_original_data$ds,
        # y = arima_predictions_total_cases_plus_original_data$y_original,
        ...
        xlim = range(arima_predictions_total_cases_plus_original_data$ds),
        ylim = c( min( na.omit(c(arima_predictions_total_cases_plus_original_data$y_original, arima_predictions_total_cases_plus_original_data$yhat_original)) ), 
                  # max( arima_predictions_total_cases_plus_original_data$y_original) * 1.1  ),
                  max( na.omit(c(arima_predictions_total_cases_plus_original_data$y_original, arima_predictions_total_cases_plus_original_data$yhat_original)) ) * 1.1  ),
      ...
      lines(
        x = arima_predictions_total_cases_plus_original_data$ds[arima_predictions_total_cases_plus_original_data$ds < war_start],
        y = arima_predictions_total_cases_plus_original_data$yhat_original[arima_predictions_total_cases_plus_original_data$ds < war_start],
        # col = "#005BBB",
        col = "#005BBB",
        # col = color_01,
        lwd = 5,
      ...
      lines(
        x = arima_predictions_total_cases_plus_original_data$ds[arima_predictions_total_cases_plus_original_data$ds >= war_start -30 ],
        y = arima_predictions_total_cases_plus_original_data$yhat_original[arima_predictions_total_cases_plus_original_data$ds >= war_start -30],
        # col = "#005BBB",
        col = "#FFD500",
        # col = color_01,
        lwd = 5,
      ...
      lines(
        ...
        y = c(rep(min(arima_predictions_total_cases_plus_original_data$y_original), 5), rep(max(arima_predictions_total_cases_plus_original_data$y_original), 5)),
        col = "red",
        lwd = 2,
        lty = 2
      )
      ...
      # labels FAQ -> http://www.r-bloggers.com/rotated-axis-labels-in-r-plots/
      # Creating labels by month and converting.
      initial_date <- min(arima_predictions_total_cases_plus_original_data$ds)
      final_date <- max(arima_predictions_total_cases_plus_original_data$ds)
      number_of_dates <- length(arima_predictions_total_cases_plus_original_data$ds)
      
      
      # Indexes to display
      x_indexes_to_display <- 
        arima_predictions_total_cases_plus_original_data$ds[seq( from  =  1, to  = length(arima_predictions_total_cases_plus_original_data$ds),  by = 5 )]
      ...
      x_lablist <- 
        dataset_current_region$period[seq( from  =  1, to  = length(arima_predictions_total_cases_plus_original_data$ds),  by = 5 )]
      ...
      y_min_value <- min( na.omit(c(arima_predictions_total_cases_plus_original_data$y_original, arima_predictions_total_cases_plus_original_data$yhat_original)) )
      y_max_value <- max( na.omit(c(arima_predictions_total_cases_plus_original_data$y_original, arima_predictions_total_cases_plus_original_data$yhat_original)) )
      ...
      # Computing p-scores
      arima_predictions_total_cases_plus_original_data$p_score <- 
        100 * (arima_predictions_total_cases_plus_original_data$y_original - arima_predictions_total_cases_plus_original_data$yhat_original) / 
        abs(arima_predictions_total_cases_plus_original_data$yhat_original)
      
      # Fixing the NaN values i.e. overwriting with 0-s
      arima_predictions_total_cases_plus_original_data$p_score[is.nan(arima_predictions_total_cases_plus_original_data$p_score)] <- 0
      
      
      # Generating year and month as text
      arima_predictions_total_cases_plus_original_data$month_text <- substr( x = as.character(arima_predictions_total_cases_plus_original_data$ds), start = 1, stop = 7)
      
      
      # Saving the results so that they can be used for p-scores
      
      # Creating objects in R.
      current_prediction_creation_text_to_run <- paste0("part02_03_arima_", infection_current, "_", region_current_name, "_", box_cox_local, " <- arima_predictions_total_cases_plus_original_data")
      
      current_prediction_saving_path <- paste0( "../R_Data/part02_03_arima_", infection_current, "_", region_current_name, "_", box_cox_local, ".RData")
      current_prediction_saving_text_to_run <- paste0("save( part02_03_arima_", infection_current, "_", region_current_name, "_", box_cox_local, ", file = current_prediction_saving_path )")
    ...
    # Generating pdf output.
    pdf(paste("../Plots/part02_03_arima_", infection_current, "_psc_box_cox_", box_cox_local, ".pdf", sep = ""), height = 12, width = 28)
    # Definign the number of plots
    par(mfrow = c(2, length(regions_unique_rearranged_short)/2 ), mar = c(5.5, 5.1, 5.1, 2.1))
    ...
    for(region_current_name in regions_unique_rearranged_short)
    {
      ...
      current_prediction_recover_text_to_run <- 
        paste0("arima_predictions_total_cases_plus_original_data <- part02_03_arima_", infection_current, "_", region_current_name, "_", box_cox_local)
      ...
      {
        frame_p_scores <- data.frame( date =  arima_predictions_total_cases_plus_original_data$ds, 
                                      current_region =  round( x= arima_predictions_total_cases_plus_original_data$p_score, digits = 2) ) 
      ...
      # Min and Max
      p_score_min <- min(na.omit(arima_predictions_total_cases_plus_original_data$p_score))
      p_score_max <- max(na.omit(arima_predictions_total_cases_plus_original_data$p_score))
      
      indexes_blue   <- which(arima_predictions_total_cases_plus_original_data$ds <  war_start)
      indexes_yellow <- which(arima_predictions_total_cases_plus_original_data$ds >= war_start) 
      
      color_codes <- rep(x = "#000000", times = length(arima_predictions_total_cases_plus_original_data$ds))
      color_codes[indexes_blue]   <- "#005BBB"
      color_codes[indexes_yellow] <- "#FFD500"
      
      
      
      barplot(arima_predictions_total_cases_plus_original_data$p_score,
              col = color_codes,
              legend = TRUE,
              border = TRUE,
              # xlim = c(1, 5),
              ...
              names.arg = arima_predictions_total_cases_plus_original_data$month_text,
    ...
    frame_p_scores_text_to_run <- paste0("part02_03_arima_", infection_current, "_pscores_combined_", box_cox_local, " <- frame_p_scores")
    
    frame_p_scores_saving_rdata_path <- paste0( "../R_Data/part02_03_arima_", infection_current, "_pscores_combined_", box_cox_local, ".RData")
    frame_p_scores_saving_text_to_run <- paste0("save( part02_03_arima_", infection_current, "_pscores_combined_", box_cox_local, ", file = frame_p_scores_saving_rdata_path )")
    ...
    # Saving csv files.
    frame_p_scores_saving_csv_path <- paste0( "../R_Output/part02_03_arima_", infection_current, "_pscores_combined_", box_cox_local, ".csv")
    ...
    # Creating a path 
    frame_p_scores_saving_tex_path <- paste0( "../R_Output/part02_03_arima_", infection_current, "_pscores_combined_", box_cox_local, ".tex")
    # Printing
...
# End of -> for (box_cox_local in box_cox_local_list )
}    
